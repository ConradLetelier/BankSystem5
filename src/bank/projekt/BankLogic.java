                ...
                for (Account account : client.getAllAccounts()) {
                    if (account.getAccountNumber()== accountId) {
                        ...
                        if(account instanceof SavingsAccount){
                            ...
                            toReturn.add("Kontonummer: " + account.getAccountNumber() + "   Saldo: " + account.getBalance() + " kr  " + account.getAcct_type() + " (1% Interest)" );
                        }
                        else{
                            
                            MainpageController.isCreditAccount = true;
                            double interest = (account.getBalance() < 0) ? -7 : 0.5;
                            toReturn.add("Kontonummer: " + account.getAccountNumber() + "   Saldo: " + account.getBalance() + " kr  CreditAccount (" + interest + "% Interest)" );
                        }
                        for(String a : account.getTransactionsList()){
                            toReturn.add(a);
                        }
                        return toReturn;
                    }
